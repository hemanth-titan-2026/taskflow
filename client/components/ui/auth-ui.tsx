...
function SignInForm({ onSubmit }: { onSubmit?: (email: string, password: string) => void }) {
  const handleSignIn = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    else console.log("UI: Sign In form submitted");
  };
...
function SignUpForm({ onSubmit }: { onSubmit?: (name: string, email: string, password: string) => void }) {
  const handleSignUp = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    else console.log("UI: Sign Up form submitted");
  };
...
function AuthFormContainer({ isSignIn, onToggle, onSignIn, onSignUp }: { isSignIn: boolean; onToggle: () => void; onSignIn?: (email: string, password: string) => void; onSignUp?: (name: string, email: string, password: string) => void; }) {
  return (
    <div className="mx-auto grid w-[350px] gap-2">
      {isSignIn ? <SignInForm onSubmit={onSignIn} /> : <SignUpForm onSubmit={onSignUp} />}
...
const defaultSignUpContent = {
  image: {
    src: "https://images.unsplash.com/photo-1534972195531-d756b9bfa9f2?w=1200&q=80",
    alt: "Code on screen"
  },
  quote: { text: "Create an account. A new chapter awaits.", author: "TaskFlow" }
};

export function AuthUI({ signInContent = {}, signUpContent = {}, onSignIn, onSignUp }: AuthUIProps) {
